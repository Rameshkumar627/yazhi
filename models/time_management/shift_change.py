...
class ShiftChange(surya.Sarpam):
    ...
    def update_attendance(self):
        ...
        attendance = self.env["time.attendance.detail"].search([("employee_id", "=", self.employee_id.id),
                                                                ("attendance_id.date", "=", self.date)])

        if attendance:
            expected_from_time = "{0} {1}:{2}:00.0".format(self.date,
                                                           self.shift_id.from_hours,
                                                           self.shift_id.from_minutes)
            if self.shift_id.end_day == 'current_day':
                expected_till_time = "{0} {1}:{2}:00.0".format(self.date,
